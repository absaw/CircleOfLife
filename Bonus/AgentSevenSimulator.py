        ...
        for trial in range(1,n_trials+1):
            ...
            while(steps<=max_steps):
                ...
                if steps>=hang_threshold:
                    n_hang+=1
                    break
                d_predator=len(get_bfs_path(agent_seven.G, agent_seven.position, survey_node)[1])  #Distance from predator
                ...
                if d_predator<pred_dist_threshold:
                    ...
                    if agent_seven.survey:
                        # Update belief system of pred only if not certain
                        agent_seven.update_belief_predator(survey_node, predator.position)
                        # Update belief system of prey only if not certain
                        agent_seven.update_belief_prey(survey_node, prey.position)
                        # print("Survey done")
                        agent_seven.survey=False
                        ...
                        # Update belief system of prey only if not certain
                        agent_seven.update_belief_prey(survey_node, prey.position)
                        agent_seven.survey=True #So that suvery is done 
                ...
                # survey_node=random.choice(survey_list) #Selecting a random element from highest prob value list
            n_sure_prey+=agent_seven.sure_of_prey
            n_sure_predator+=agent_seven.sure_of_predator
