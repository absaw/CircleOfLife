...
def simulate_agent_nine():
    #=========== Log file =======================
    start = time()
    filename_txt="Results/AgentNine.txt"
    filename_csv="Results/AgentNine.csv"
    ...
    for sim in range(1,n_sim+1):
        ...
        for trial in range(1,n_trials+1):
            ...
            while(steps<=max_steps):
                ...
                if agent_nine.position==predator.position:
                    n_lose+=1
                    break
                ...
                agent_nine.update_belief_prey(survey_node, prey.position,True)
                agent_nine.update_belief_predator(survey_node, predator.position,False)
                ...
                agent_nine.update_belief_prey(agent_nine.position, prey.position,False)
                agent_nine.update_belief_predator(agent_nine.position, predator.position,False)

                # ======== Prey Simulation   =========
                prey.simulate_step()
                agent_nine.transition_update_prey()
                agent_nine.p_now_prey=agent_nine.p_next_prey.copy()

                #agent_nine.print_sum()
                ...
                # ======== Predator Simulation   =========
                predator.simulate_step_distracted(agent_nine.position)
                # New Info : Predator has moved. So update apply transition probability update to each node in graph
                agent_nine.transition_update_predator()
                agent_nine.p_now_predator=agent_nine.p_next_predator.copy()
                #agent_nine.print_sum()
                ...
                if agent_nine.position==prey.position:
                    ...
                    n_steps+=steps
                    break
                # find_survey_node=True
                if agent_nine.recheck_survey_node:
                    if agent_nine.n_rechecks<=2:
                    #Keep survey node same and check again in the next iteration
                        continue
                    else:
                        agent_nine.recheck_survey_node=False
                        agent_nine.n_rechecks=0

                # if find_survey_node:
                is_pred_certain=False
                for p_now_index in range(len(agent_nine.p_now_predator)):
                    if agent_nine.p_now_predator[p_now_index] == 1:
                        is_pred_certain=True
                        # survey_node=p_now_index+1
                if is_pred_certain:
                    # Now we calculate max survey node based on prey's probability
                    m=max(agent_nine.p_now_prey) #finding value with highest prob
                    survey_list=[node+1 for node in range(len(agent_nine.p_now_prey)) if agent_nine.p_now_prey[node]==m] # List of nodes with highest prob value
                    survey_node=random.choice(survey_list) #Selecting a random element from highest prob value list
                if not is_pred_certain:

                    m=max(agent_nine.p_now_predator) #finding value with highest prob
                    max_prob_list=[node+1 for node in range(len(agent_nine.p_now_predator)) if agent_nine.p_now_predator[node]==m] # List of nodes with highest prob value
                    ...
                    for max_prob_node in max_prob_list:
                        distance_from_agent_list.append(len(get_bfs_path(G, max_prob_node, agent_nine.position)[1]))
