...
class AgentThree:
    ...
    def update_belief(self,survey_node,prey_positon):
        ...
        if survey_node==prey_positon:
            ...
            print("After locating prey -> ",*self.p_next)
        else:
            # self.G.nodes[survey_node]["P_now"]=0
            print("Survey node:",survey_node)
            # print("Current node:",survey_node)
            p_new=[0]*50
            ...
            for node in range(1,51):
                if node!=survey_node:
                    # p_prey_not_in_survey_node=1-self.G.nodes[survey_node]["P_now"]
                    # p_prey_in_current_node=self.G.nodes[node]["P_now"]
    ...
    def transition_update(self):
        for survey_node in range(1,n_nodes+1):
